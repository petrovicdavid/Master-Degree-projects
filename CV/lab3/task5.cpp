...
void onMouse(int event, int x, int y, int f, void* userdata){
  ...
  if(event == EVENT_LBUTTONDOWN)
  {
    ...
    if(y + NEIGHBORHOOD > image.rows || x + NEIGHBORHOOD > image.cols){
      return;
    }
    ...
    ref_b = (float) sum_b/cont;
    ref_g = (float) sum_g/cont;
    ref_r = (float) sum_r/cont;
    ...
    for(int i=0;i<image.rows;i++){
      for(int j=0;j<image.cols;j++){
        ...
        if(dist_b <= T && dist_g <= T && dist_r <= T){
					...
					mask.at<uchar>(i, j) = 0; // set black pixel
        }
      }
    }
    ...
    for(int i=0;i<new_image.rows;i++){
      for(int j=0;j<new_image.cols;j++){
				uchar pixel_mask = mask.at<uchar>(i,j);
        if(pixel_mask == 255){ //if the pixel of the mask is white
          Vec3b color(92, 37, 201); //given color
					new_image.at<Vec3b>(i, j) = color; // set given triplet pixel
        }
      }
    }

    namedWindow( "Display new image");
